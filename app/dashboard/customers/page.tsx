...
export default async function CustomersPage(props: { searchParams?: Promise<{ q?: string; page?: string; pageSize?: string; showDeleted?: string }> }) {
  ...
  const customers = rawCustomers.map((c) => ({
    id: c.id, name: c.name, email: c.email, phone: c.phone, deletedAt: c.deletedAt?.toISOString() ?? null, createdAt: c.createdAt.toISOString(),
  }))

  return <CustomersClient initialCustomers={customers} total={total} page={page} pageSize={pageSize} userRole={session.user.role} showDeleted={showDeleted} />
}
