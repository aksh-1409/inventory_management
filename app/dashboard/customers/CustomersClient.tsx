...
export default function CustomersClient({ initialCustomers, total, page, pageSize, userRole, showDeleted }: Props) {
  ...
  const [optimisticCustomers, addOptimistic] = useOptimistic(customers, (state, action: OptimisticAction) => {
    switch (action.type) {
      case 'create': return [...state, action.item]
      case 'update': return state.map((c) => (c.id === action.id ? { ...c, ...action.updates } : c))
      case 'delete': return state.filter((c) => c.id !== action.id)
      case 'restore': return state.map((c) => (c.id === action.id ? { ...c, deletedAt: null } : c))
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (editing) {
      startTransition(async () => {
        ...
        try {
          const res = await fetch(`/api/v1/customers/${editing.id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(validated) })
      ...
      startTransition(async () => {
        ...
        try {
          const res = await fetch('/api/v1/customers', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(validated) })
          const data = await res.json()
          if (!res.ok) throw new Error(data.error)
    ...
    <div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24, flexWrap: 'wrap', gap: 12 }}>
        <div>
          ...
          <p style={{ fontSize: 14, color: 'var(--text-muted)', marginTop: 4 }}>{total} customers</p>
        </div>
                    ...
                    <tr key={c.id} style={{ borderBottom: '1px solid var(--border)', opacity: deleted ? 0.5 : 1 }}>
                      ...
                      <td style={{ padding: '16px 24px', textAlign: 'right' }}>
                        <div style={{ display: 'flex', gap: 4, justifyContent: 'flex-end' }}>
                          {deleted ? (
                            <button onClick={() => handleRestore(c.id)} className="btn btn-ghost" style={{ padding: 8, minHeight: 'auto', minWidth: 'auto', color: 'var(--success)' }} title="Restore">
          ...
          <div className="surface-2" style={{ position: 'relative', width: '100%', maxWidth: 420, borderRadius: 12, border: '1px solid var(--border)', padding: 24 }}>
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <div>
                ...
                <input className="input" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} placeholder="+1-555-000-0001" />
              ...
              <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', marginTop: 8 }}>
                ...
                <button type="submit" disabled={isPending} className="btn btn-primary">
                  {isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : editing ? 'Save Changes' : 'Create Customer'}
                </button>
              </div>
            </form>
          </div>
