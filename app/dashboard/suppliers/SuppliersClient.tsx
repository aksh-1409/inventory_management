...
export default function SuppliersClient({
  ...
  const [optimisticSuppliers, addOptimistic] = useOptimistic(
    suppliers,
    (state, action: OptimisticAction) => {
      switch (action.type) {
        ...
        case 'restore':
          return state.map((s) => (s.id === action.id ? { ...s, deletedAt: null } : s));
        default:
          return state;
      }
    }
  ...
  function openEdit(s: SupplierData) {
    setEditing(s);
    setForm({
      name: s.name,
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (editing) {
      startTransition(async () => {
        ...
        try {
          const res = await fetch(`/api/v1/suppliers/${editing.id}`, {
            method: 'PATCH',
          ...
          setSuppliers((prev) =>
            prev.map((s) => (s.id === editing.id ? { ...s, ...data.supplier } : s))
          );
          ...
          if (suppliers.length === 0) setError(message);
        }
      });
    } else {
      startTransition(async () => {
        ...
        try {
          ...
          setSuppliers((prev) => [
            ...prev,
            { ...data.supplier, createdAt: data.supplier.createdAt },
          ]);
  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        <EmptyState
          icon={Truck}
          title={qParam ? 'No suppliers match your search' : 'No suppliers found'}
          description={
            qParam
        ...
        <EmptyState
          ...
          description="All suppliers are active. Toggle off 'Show Deleted' to return."
        />
      ) : (
        <>
          ...
          <div style={{ marginBottom: 12 }}>
            <input
              ...
              checked={
                ...
                (suppliers.length > 0 && selection.selectedIds.size === suppliers.length)
              }
              onChange={() => {
            ...
            <span style={{ fontSize: 13, color: 'var(--text-muted)', marginLeft: 8 }}>
              {selection.selectionCount > 0 ? `${selection.selectionCount} selected` : 'Select all'}
            </span>
          </div>
          <div
            style={{
