    ...
    <div>
      ...
      {error && filtered.length === 0 ? (
        ...
        <EmptyState
          icon={Webhook}
          title="No webhooks"
          description={search ? 'Try a different search.' : 'Add a webhook to get notified when events happen.'}
        />
      ) : (
            ...
            <form onSubmit={handleCreate} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div>
                ...
                <select
                  ...
                  required
                  style={{ width: '100%', background: 'rgba(255,255,255,0.05)', border: '1px solid var(--border)', borderRadius: 8, padding: '10px 16px', color: 'var(--text-heading)', fontSize: 14, cursor: 'pointer' }}
                >
                  <option value="">Select event...</option>
                  {EVENT_TYPES.map((et) => (
                    <option key={et.value} value={et.value} style={{ background: '#1a1a24' }}>{et.label}</option>
                  ))}
                </select>
              </div>
              <div>
