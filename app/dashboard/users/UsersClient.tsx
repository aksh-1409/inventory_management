      ...
      <div className="card" style={{ padding: 20, marginBottom: 24 }}>
        <div
          style={{
            ...
            alignItems: 'center',
            justifyContent: 'space-between',
            marginBottom: requests.length ? 16 : 0,
          }}
        >
          <div>
            <h2 style={{ color: 'var(--text-heading)', fontSize: 16, fontWeight: 600 }}>
          ...
          <Clock3
            style={{ width: 18, color: requests.length ? 'var(--accent)' : 'var(--text-muted)' }}
          />
        </div>
        {requests.length === 0 ? (
          <p style={{ color: 'var(--text-muted)', fontSize: 13 }}>
            No operators are waiting for a password reset.
          </p>
        ) : (
          requests.map((request) => (
            ...
            >
              <div>
                <p style={{ color: 'var(--text-heading)', fontSize: 14, fontWeight: 500 }}>
                  {request.operatorName}
                </p>
                <p style={{ color: 'var(--text-muted)', fontSize: 12 }}>
                  {request.operatorEmail} · {request.warehouseName || 'No warehouse'}
                </p>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                ...
                >
                  {reviewing === request.id ? (
                    <Loader2 style={{ width: 14, animation: 'spin 1s linear infinite' }} />
                  ) : (
                  ...
                  <tr key={u.id} style={{ borderBottom: '1px solid var(--border)' }}>
                    ...
                    <td data-label="Role" style={{ padding: '16px 24px', textAlign: 'center' }}>
                      <span
                        ...
                        style={{
                          ...
                          gap: 4,
                          background:
                            u.role === 'ADMIN' ? 'rgba(99,102,241,0.12)' : 'rgba(255,255,255,0.06)',
                    ...
                    <td data-label="Warehouse" style={{ padding: '16px 24px' }}>
                      <span
                        style={{
                          fontSize: 13,
                          color: u.warehouse ? 'var(--text-secondary)' : 'var(--text-muted)',
                        }}
                      >
                        {u.warehouse?.name || '—'}
                      </span>
