...
export default function TransfersClient({
    ...
    (state, action: OptimisticAction) => {
      switch (action.type) {
        ...
        case 'accept':
          return state.map((t) =>
            t.id === action.id
              ? { ...t, status: 'PENDING', fromWarehouse: action.fromWarehouse }
              : t
          );
        case 'ship':
          return state.map((t) =>
          ...
          return state.map((t) =>
            t.id === action.id
              ? {
                  ...
                  damagedQuantity: action.damagedQuantity,
  ...
  const tabbed = useMemo(() => {
    if (tab === 'incoming') {
      return optimisticTransfers.filter(
        (t) => t.status === 'REQUESTED' && (isAdmin || t.toWarehouse.id === userWarehouseId)
      );
    } else if (tab === 'fulfill') {
      return optimisticTransfers.filter(
        (t) => t.status === 'REQUESTED' && t.toWarehouse.id !== userWarehouseId
      );
    }
    ...
    startTransition(async () => {
      ...
      addOptimistic({
        ...
        transfer: {
          ...
          toWarehouse: {
            id: destinationId,
            name: warehouses.find((w) => w.id === destinationId)?.name || '',
          },
  ...
  async function handleAccept(transferId: string) {
    ...
    startTransition(async () => {
      addOptimistic({
        ...
        fromWarehouse: {
          id: sourceId,
          name: warehouses.find((w) => w.id === sourceId)?.name || '',
        },
      });
  ...
  async function handleShip(id: string) {
    ...
    startTransition(async () => {
      ...
      try {
          ...
          prev.map((t) =>
            t.id === id
              ? {
                  ...t,
                  status: 'IN_TRANSIT',
                  shippedAt: data.transfer.shippedAt,
                  trackingNumber: data.transfer.trackingNumber,
                }
              : t
          )
        );
        ...
        showToast(err instanceof Error ? err.message : 'Failed to ship', 'error');
      }
    });
  }
          ...
          prev.map((t) =>
            t.id === id
              ? {
                  ...
                  quantityReceived: result.data.quantityReceived,
                  damagedQuantity: result.data.damagedQuantity || 0,
                  receivedAt: data.transfer.receivedAt,
                }
              : t
          )
        );
  ...
  async function handleLoadMore() {
    ...
    try {
      ...
      if (transfers.length === 0) setError(message);
    } finally {
  ...
  const fulfillCount = transfers.filter(
    (t) => t.status === 'REQUESTED' && t.toWarehouse.id !== userWarehouseId
  ).length;
      ...
      >
        <div>
          ...
          <p style={{ fontSize: 13, color: 'var(--text-muted)', marginTop: 4 }}>
            {isAdmin
              ? `${cursorMode ? totalCount : total} total`
              : `${userWarehouseName} &middot; ${cursorMode ? totalCount : total} total`}
          </p>
        </div>
        <button
          onClick={() => {
        ...
        <EmptyState
          ...
          title={
            tab === 'incoming'
              ...
              : tab === 'fulfill'
                ? qParam
                  ? 'No requests to fulfill match your search'
          ...
          description={
            qParam
              ...
              : 'Try a different search or create a new transfer request.'
          }
        />
      ) : (
        <>
          ...
          >
            {tabbed.map((t) => {
              ...
              const canShip =
                !isRequest &&
                t.status === 'PENDING' &&
                (isAdmin || t.fromWarehouse?.id === userWarehouseId);
              const canReceive =
                !isRequest &&
                t.status === 'IN_TRANSIT' &&
                (isAdmin || t.toWarehouse.id === userWarehouseId);
              const canDownloadPackingSlip =
                !isRequest &&
                (t.status === 'PENDING' || t.status === 'IN_TRANSIT') &&
                (isAdmin || t.fromWarehouse?.id === userWarehouseId);
              const canDownloadReceivingReport =
                t.status === 'COMPLETED' && (isAdmin || t.toWarehouse.id === userWarehouseId);
                  ...
                  >
                    ...
                    >
                      ...
                      {canDownloadReceivingReport && (
                        <ReceivingReportDownload
                          transfer={{
                            ...
                            damagedQuantity: t.damagedQuantity,
                      ...
                      {canFulfill && (
                        <button
                          onClick={() => {
                            setShowAcceptModal(t.id);
                            setAcceptForm({ fromWarehouseId: userWarehouseId || '' });
                          }}
                          className="btn btn-primary"
                      ...
                      {canReceive && (
                        <button
                          onClick={() => {
                            setShowReceiveModal(t.id);
                            setReceiveForm({
                              quantityReceived: t.quantityInitiated.toString(),
                              damagedQuantity: '0',
                              notes: '',
                            });
                            setReceiveErrors({});
        ...
        >
          ...
          >
            ...
            <p style={{ fontSize: 13, color: 'var(--text-secondary)', marginBottom: 16 }}>
              Stock will be deducted from {isAdmin ? 'the selected' : 'your'} warehouse and shipped
              to the requester.
            </p>
            <form
              onSubmit={(e) => {
                e.preventDefault();
                handleAccept(showAcceptModal);
              }}
              style={{ display: 'flex', flexDirection: 'column', gap: 16 }}
            >
              {isAdmin && (
        ...
        >
          ...
          >
            ...
            <form
              onSubmit={(e) => {
                e.preventDefault();
                handleReceive(showReceiveModal);
              }}
              style={{ display: 'flex', flexDirection: 'column', gap: 16 }}
            >
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setReceiveForm({ ...receiveForm, quantityReceived: e.target.value })
                  }
                />
              ...
              <div>
                ...
                <input
                  ...
                  type="number"
                  min="0"
                  value={receiveForm.damagedQuantity}
                  onChange={(e) =>
                    setReceiveForm({ ...receiveForm, damagedQuantity: e.target.value })
                  }
                />
                {receiveErrors.damagedQuantity && (
                  <p style={{ fontSize: 12, color: 'var(--danger)', marginTop: 4 }}>
                    {receiveErrors.damagedQuantity}
                  </p>
                )}
                <p style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 4 }}>
                  Damaged items will be written off from inventory.
                </p>
              </div>
              <div>
                <label
