...
export default async function ApiKeysPage(props: { searchParams?: Promise<{ q?: string; page?: string; pageSize?: string }> }) {
  ...
  const where = { userId: session.user.id, ...(q ? { name: { contains: q, mode: 'insensitive' as const } } : {}) }
  ...
  const keys = rawKeys.map((k) => ({
    ...
    scopes: k.scopes, isActive: k.isActive,
