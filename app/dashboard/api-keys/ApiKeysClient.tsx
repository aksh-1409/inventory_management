...
export default function ApiKeysClient({ initialKeys }: Props) {
  ...
  const filtered = useMemo(() => {
    ...
    return keys.filter(
      (k) => k.name.toLowerCase().includes(q) || k.scopes.some((s) => s.includes(q))
    );
  }, [keys, search]);
  ...
  return (
    <div>
      ...
      {newKeyVisible && (
        <div
          className="card"
          style={{ padding: 20, marginBottom: 20, border: '1px solid var(--success)' }}
        >
          <div
            style={{
              display: 'flex',
          ...
          >
            <div>
              <p
                style={{ fontSize: 14, fontWeight: 600, color: 'var(--success)', marginBottom: 8 }}
              >
                Key Created
              </p>
        ...
        <EmptyState
          ...
          description={
            search
              ...
              : 'Create an API key to access StockPilot programmatically.'
          }
        />
      ) : (
              ...
              >
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                    <span style={{ fontSize: 14, fontWeight: 500, color: 'var(--text-heading)' }}>
                      {k.name}
                    </span>
                    <span
                      className="badge"
                      style={{
                        background: k.isActive ? 'rgba(52,211,153,0.12)' : 'rgba(239,68,68,0.12)',
                        color: k.isActive ? 'var(--success)' : 'var(--danger)',
                        fontSize: 12,
                      }}
                    >
                ...
                {k.isActive && (
                  <button
                    ...
                    disabled={loading}
                    className="btn btn-ghost"
                    style={{ gap: 4, color: 'var(--danger)', fontSize: 12, flexShrink: 0 }}
                  >
                      ...
                      <button
                        ...
                        onClick={() => {
                          setCreateForm({
                            ...createForm,
                            scopes: selected
                              ? createForm.scopes.filter((x) => x !== s)
                              : [...createForm.scopes, s],
                          });
                        }}
                        style={{
