...
export default function InventoryClient({ initialItems, total, page, pageSize, totalCount, nextCursor: initialNextCursor, hasMore: initialHasMore, userRole }: Props) {
  ...
  const [optimisticItems, addOptimistic] = useOptimistic(items, (state, action: OptimisticAction) => {
    ...
    <div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24, flexWrap: 'wrap', gap: 12 }}>
        <div>
          ...
          <p style={{ fontSize: 14, color: 'var(--text-muted)', marginTop: 4 }}>{cursorMode ? totalCount : total} items across all warehouses</p>
        </div>
      </div>
      ...
      {optimisticItems.length === 0 ? (
        <EmptyState
          ...
          description={qParam ? 'Try a different search term or clear the filter.' : 'Try a different search or receive stock first.'}
                          ...
                          <td style={{ padding: '16px 24px', textAlign: 'center' }}>
                            {adjusting === item.id ? (
                              <div style={{ display: 'flex', flexDirection: 'column', gap: 8, alignItems: 'center', minWidth: 160 }}>
                                ...
                                {errors.delta && <p style={{ fontSize: 11, color: 'var(--danger)' }}>{errors.delta}</p>}
                                {errors.reason && <p style={{ fontSize: 11, color: 'var(--danger)' }}>{errors.reason}</p>}
                                <div style={{ display: 'flex', gap: 4 }}>
                                  <button onClick={() => handleAdjust(item.id)} disabled={isPending} style={{ padding: '4px 8px', fontSize: 12, background: 'var(--success)', color: '#022c22', border: 'none', borderRadius: 6, cursor: 'pointer', fontWeight: 500 }}>
