...
export default async function InventoryPage(props: { searchParams?: Promise<{ q?: string; cursor?: string; take?: string; page?: string; pageSize?: string }> }) {
  ...
  async function fetchItems(skipVal?: number, takeVal?: number, cursorVal?: string) {
    return prisma.inventoryItem.findMany({
      ...(cursorVal ? { take: takeVal! + 1, cursor: { id: cursorVal }, skip: 1 } : { skip: skipVal, take: takeVal }),
      where,
      include: {
        product: true,
        warehouse: true,
      ...
      orderBy: cursorVal ? [{ product: { name: 'asc' } }, { id: 'asc' }] : { product: { name: 'asc' } },
    })
  }
  ...
  if (cursorMode) {
    ...
    const items = rawItems.map((item) => ({
      ...
      product: { id: item.product.id, sku: item.product.sku, name: item.product.name, reorderPoint: item.product.reorderPoint },
    ...
    return <InventoryClient initialItems={data} totalCount={totalCount} nextCursor={nextCursor} hasMore={hasMore} userRole={session.user.role} />
  }
