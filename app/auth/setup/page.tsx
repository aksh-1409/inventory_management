...
export default function SetupPage() {
  ...
  useEffect(() => {
    ...
    getSession().then((session) => {
      ...
      setNeedsPassword(!session?.user?.passwordSetAt);
    });
  }, []);
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <>
              <div>
                ...
                <p style={{ fontSize: 11, color: 'rgba(161,161,170,0.5)', marginTop: 4 }}>
                  Use this password when signing in without Google or GitHub.
                </p>
              </div>
              <div>
          ...
          <div>
            <label style={labelStyle}>
              Warehouse
              {userRole === 'ADMIN' && (
                <span style={{ color: 'rgba(161,161,170,0.5)', fontWeight: 400 }}>
                  {' '}
                  (optional for admins)
                </span>
              )}
            ...
            >
              ...
              {warehouses.map((w) => (
                <option key={w.id} value={w.id} style={{ background: '#1a1a1a' }}>
                  {w.name}
                </option>
              ))}
            </select>
