...
export default function PasswordResetPage() {
  ...
  useEffect(() => {
    ...
    async function poll() {
      try {
        const response = await fetch('/api/auth/password-reset-request/status', {
          cache: 'no-store',
        });
    ...
    >
      <section className="card" style={{ width: '100%', maxWidth: 440, padding: 32 }}>
        ...
        {status === 'PENDING' && (
          <StatusMessage
            icon={Clock3}
            title="Waiting for administrator"
            text="Keep this page open. It will update automatically when an administrator reviews your request."
          />
        )}
        {status === 'REJECTED' && (
          <StatusMessage
            ...
            text="An administrator rejected your password reset request."
            danger
          />
        )}
        {(status === 'EXPIRED' || status === 'CANCELLED') && (
          <StatusMessage
            ...
            text="Submit a new password reset request to continue."
            danger
          />
        )}
        {status === 'COMPLETED' && (
          <StatusMessage
            ...
            text="Redirecting you to sign in with your new password."
          />
        )}
        ...
        {status === 'APPROVED' && (
          <form onSubmit={completeReset}>
            <StatusMessage
              ...
              text="Create your new StockPilot password."
            />
            {error && (
        ...
        {terminal && (
          <Link
            ...
            style={{ display: 'inline-block', marginTop: 18, color: 'var(--accent)', fontSize: 13 }}
          >
...
function StatusMessage({
  ...
  return (
    <div style={{ marginBottom: 20 }}>
      <Icon
        style={{ width: 28, color: danger ? 'var(--danger)' : 'var(--accent)', marginBottom: 10 }}
      />
