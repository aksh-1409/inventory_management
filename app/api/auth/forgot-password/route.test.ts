...
vi.mock('crypto', () => ({
  default: {
    ...
    createHash: vi.fn().mockReturnValue({ update: vi.fn().mockReturnValue({ digest: vi.fn().mockReturnValue('hash') }) }),
  },
}))
vi.mock('@/lib/prisma', () => ({ prisma: { user: { findUnique: mocks.findUnique }, $transaction: mocks.transaction, passwordResetRequest: { updateMany: mocks.updateMany, create: mocks.create } } }))
vi.mock('@/lib/email', () => ({ normalizeEmail: (e: string) => e.toLowerCase().trim() }))

import { POST } from './route'

const request = (body: unknown, ip?: string) => new NextRequest('http://localhost/api/auth/forgot-password', {
  method: 'POST',
...
describe('POST /api/auth/forgot-password', () => {
  ...
  it('creates a request for OPERATOR and sets a cookie', async () => {
    mocks.findUnique.mockResolvedValue({ id: 'u1', email: 'op@test.com', role: 'OPERATOR', name: 'Op' })
