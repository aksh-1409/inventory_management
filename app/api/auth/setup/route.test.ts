...
vi.mock('@/lib/prisma', () => ({ prisma: { user: { findUnique: mocks.findUnique, update: mocks.userUpdate }, warehouse: { findUnique: mocks.whFindUnique } } }))
vi.mock('bcryptjs', () => ({ default: { hash: mocks.hash } }))

import { PUT } from './route'

const request = (body: unknown) => new NextRequest('http://localhost/api/auth/setup', {
  method: 'PUT',
...
describe('PUT /api/auth/setup', () => {
  beforeEach(() => {
    ...
    mocks.findUnique.mockResolvedValue({ id: 'u1', name: 'Old Name', role: 'ADMIN', passwordSetAt: new Date() })
