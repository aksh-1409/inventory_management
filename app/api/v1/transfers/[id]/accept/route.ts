  ...
  try {
    ...
    if (user.role === 'OPERATOR' && user.warehouseId !== fromWarehouseId) {
      return NextResponse.json(
        { error: 'You can only accept requests from your assigned warehouse' },
        { status: 403 }
      );
    }
    ...
    const [transfer] = await prisma.$transaction(async (tx) => {
      ...
      if (existing.status !== 'REQUESTED')
        throw new Error(`CONFLICT:Cannot accept transfer in ${existing.status} status`);
      ...
      const sourceInv = await tx.inventoryItem.findUnique({
        where: {
          productId_warehouseId: { productId: existing.productId, warehouseId: fromWarehouseId },
        },
      });
      if (!sourceInv || sourceInv.quantity < existing.quantityInitiated) {
        throw new Error(
          `CONFLICT:Insufficient stock at source. Available: ${sourceInv?.quantity || 0}, requested: ${existing.quantityInitiated}`
        );
      }
