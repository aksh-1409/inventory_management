...
export async function POST(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    const preCheck = await prisma.transfer.findUnique({
      where: { id },
      select: { toWarehouseId: true, quantityInitiated: true },
    });
    ...
    if (user.role === 'OPERATOR' && user.warehouseId !== preCheck.toWarehouseId) {
      return NextResponse.json(
        { error: 'Only the destination warehouse can receive this transfer' },
        { status: 403 }
      );
    }
    ...
    const [transfer] = await prisma.$transaction(async (tx) => {
      ...
      if (existing.status !== 'IN_TRANSIT')
        throw new Error(`CONFLICT:Cannot receive transfer in ${existing.status} status`);
