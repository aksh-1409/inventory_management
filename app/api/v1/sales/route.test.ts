...
const mocks = vi.hoisted(() => ({
  requireAuth: vi.fn(), findInventory: vi.fn(), updateInventory: vi.fn(),
  createTransaction: vi.fn(), transaction: vi.fn(), findProduct: vi.fn(),
  findWarehouse: vi.fn(), findCustomer: vi.fn(),
}))
...
vi.mock('@/lib/prisma', () => ({ prisma: {
  ...
  inventoryTransaction: { create: mocks.createTransaction }, $transaction: mocks.transaction,
  product: { findUnique: mocks.findProduct }, warehouse: { findUnique: mocks.findWarehouse },
  customer: { findUnique: mocks.findCustomer },
} }))

import { POST } from './route'

const request = (body: unknown) => new NextRequest('http://localhost/api/v1/sales', {
  method: 'POST', body: JSON.stringify(body), headers: { 'content-type': 'application/json' },
})
const validSale = { productId: 'p1', warehouseId: 'w1', customerId: 'c1', quantity: 2, unitPrice: 10 }
...
describe('POST /api/v1/sales', () => {
  beforeEach(() => {
    ...
    mocks.createTransaction.mockReturnValue(Promise.resolve({ id: 't1', createdAt: new Date('2026-01-01') }))
  ...
  it('records a sale at the exact available-stock boundary', async () => {
    ...
    await expect(response.json()).resolves.toMatchObject({ sale: { quantity: 2, total: 20 }, remainingStock: 0 })
    expect(mocks.updateInventory).toHaveBeenCalledWith({ where: { id: 'i1' }, data: { quantity: { decrement: 2 } } })
  })
  ...
  it('prevents operators from selling stock in another warehouse', async () => {
    mocks.requireAuth.mockResolvedValue({ user: { id: 'u1', role: 'OPERATOR', warehouseId: 'w2' } })
    const response = await POST(request(validSale))
