...
const mocks = vi.hoisted(() => ({
  requireAuth: vi.fn(), product: vi.fn(), warehouse: vi.fn(), supplier: vi.fn(),
  upsert: vi.fn(), update: vi.fn(), createTransaction: vi.fn(), transaction: vi.fn(),
}))
vi.mock('@/lib/api-auth', () => ({ requireAuth: mocks.requireAuth, hasScope: vi.fn().mockReturnValue(true) }))
vi.mock('@/lib/prisma', () => ({ prisma: {
  product: { findUnique: mocks.product }, warehouse: { findUnique: mocks.warehouse },
  supplier: { findUnique: mocks.supplier }, inventoryItem: { upsert: mocks.upsert, update: mocks.update },
...
const request = (value: unknown) => new NextRequest('http://localhost/api/v1/receive', {
...
describe('POST /api/v1/receive', () => {
  beforeEach(() => {
    ...
    mocks.transaction.mockResolvedValue([{ quantity: 1 }, { id: 't1', createdAt: new Date('2026-01-01') }])
  })

  it('receives the minimum quantity and calculates cost', async () => {
    const response = await POST(request(body))
    expect(response.status).toBe(201)
    await expect(response.json()).resolves.toMatchObject({ receipt: { quantity: 1, total: 4 }, newStock: 1 })
  })
