...
export function csvStream(columns: ColumnDef[], rows: AsyncGenerator<Record<string, unknown>> | Record<string, unknown>[]) {
  return new Response(
    new ReadableStream({
      async start(controller) {
        ...
        for await (const row of iter) {
          ...
          for (const col of columns) {
            transformed[col.key] = col.transform ? col.transform(row[col.key]) : (row[col.key] ?? '')
          }
