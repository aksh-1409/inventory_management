...
const { findManyMock, updateMock } = vi.hoisted(() => ({ findManyMock: vi.fn(), updateMock: vi.fn() }))
vi.mock('@/lib/prisma', () => ({
...
describe('dispatchWebhook', () => {
  ...
  it('delivers signed JSON and resets retries after success', async () => {
    ...
    findManyMock.mockResolvedValue([{ id: 'sub-1', targetUrl: 'https://example.com/hook', secret: 'secret', retryCount: 2 }])
    vi.mocked(fetch).mockResolvedValue(new Response(null, { status: 204 }))

    await dispatchWebhook('sale.created', { id: 'sale-1' })

    const payload = JSON.stringify({ event: 'sale.created', timestamp: '2026-01-01T00:00:00.000Z', data: { id: 'sale-1' } })
    ...
    expect(updateMock).toHaveBeenCalledWith(expect.objectContaining({ data: expect.objectContaining({ retryCount: 0 }) }))
    vi.useRealTimers()
  })
  ...
  it('increments retries atomically for HTTP and network failures', async () => {
    ...
    expect(updateMock).toHaveBeenCalledWith(expect.objectContaining({
      where: { id: 'http' }, data: expect.objectContaining({ retryCount: { increment: 1 } }),
    }))
    expect(updateMock).toHaveBeenCalledWith({ where: { id: 'network' }, data: { retryCount: { increment: 1 } } })
  })
