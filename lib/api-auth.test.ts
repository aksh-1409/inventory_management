...
const request = (key?: string) =>
  new NextRequest('http://localhost/api/v1/products', {
...
describe('API authentication', () => {
  ...
  it('authenticates an active unexpired API key and records its use', async () => {
    findUniqueMock.mockResolvedValue({
      ...
      user: { role: 'OPERATOR', warehouseId: 'warehouse-1' },
    });
  ...
  it('prefers a session and falls back to an API key', async () => {
    authMock.mockResolvedValueOnce({
      user: { id: 'session-user', role: 'ADMIN', warehouseId: null },
    });
    ...
    findUniqueMock.mockResolvedValueOnce({
      ...
      expiresAt: null,
      scopes: [],
      user: { role: 'OPERATOR', warehouseId: null },
    });
  ...
  it('enforces exact scopes while allowing administrators', () => {
    ...
    expect(
      hasScope(
        { id: '1', role: 'OPERATOR', warehouseId: null, scopes: ['products:read'] },
        'products:read'
      )
    ).toBe(true);
    ...
    expect(
      hasScope(
        { id: '1', role: 'OPERATOR', warehouseId: null, scopes: ['products'] },
        'products:read'
      )
    ).toBe(false);
    // API-key user with empty scopes → denied
    expect(
      hasScope({ id: '1', role: 'OPERATOR', warehouseId: null, scopes: [] }, 'products:read')
    ).toBe(false);
