...
function parseCombo(combo: KeyCombo) {
  ...
  const key = parts.filter(p => !['ctrl', 'meta', 'mod', 'cmd', 'alt', 'shift'].includes(p))[0] || ''
  return { key, ctrlOrMeta: hasCtrl || hasMeta || hasCmd, alt: parts.includes('alt'), shift: parts.includes('shift') }
}
