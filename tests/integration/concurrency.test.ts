...
async function seed() {
  ...
  const operator = await prisma.user.create({
    data: { name: 'Test Op', email: 'op@test.local', passwordHash: 'x', role: 'OPERATOR', warehouseId: null },
  })
  ...
  try {
    const result = await prisma.$transaction(async (tx) => {
      ...
      await tx.inventoryTransaction.create({
        data: { inventoryItemId: inv.id, type: 'OUT', delta: -qty, reference: 'Concurrent sale test' },
      })
...
async function raceSales(productId: string, warehouseId: string, qtyA: number, qtyB: number) {
  const [ra, rb] = await Promise.all([trySale(productId, warehouseId, qtyA), trySale(productId, warehouseId, qtyB)])
...
async function tryAccept(transferId: string, fromWhId: string): Promise<{ ok: boolean; status: number }> {
  try {
    return await prisma.$transaction(async (tx) => {
      ...
      if (!srcInv || srcInv.quantity < existing.quantityInitiated) return { ok: false, status: 409 }

      await tx.transfer.update({ where: { id: transferId }, data: { fromWarehouseId: fromWhId, status: 'PENDING' } })
      await tx.inventoryItem.update({ where: { id: srcInv.id }, data: { quantity: { decrement: existing.quantityInitiated } } })
      await tx.inventoryTransaction.create({
        data: { inventoryItemId: srcInv.id, type: 'TRANSFER_OUT', delta: -existing.quantityInitiated, reference: 'Concurrent accept test' },
      })
...
async function raceAccepts(transferId: string, srcWhId: string, prodId: string) {
  const [ra, rb] = await Promise.all([tryAccept(transferId, srcWhId), tryAccept(transferId, srcWhId)])
  ...
  try {
    return await prisma.$transaction(async (tx) => {
      ...
      await tx.transfer.update({
        where: { id: transferId },
        data: { status: 'COMPLETED', quantityReceived: existing.quantityInitiated, receivedAt: new Date() },
      })
      await tx.inventoryItem.update({ where: { id: destInv.id }, data: { quantity: { increment: existing.quantityInitiated } } })
      await tx.inventoryTransaction.create({
        data: { inventoryItemId: destInv.id, type: 'TRANSFER_IN', delta: existing.quantityInitiated, reference: 'Concurrent receive test' },
      })
      return { ok: true, status: 200 }
    })
  } catch { return { ok: false, status: 500 } }
...
describe('real-database concurrency and invariants', () => {
  ...
  it('prevents duplicate concurrent accept of the same REQUESTED transfer', async () => {
    ...
    const { ra, rb, finalTransfer, finalInv } = await raceAccepts(data.transfer.id, data.w1.id, data.prod.id)
  ...
  it('prevents duplicate concurrent receive of the same IN_TRANSIT transfer', async () => {
    ...
    const { ra, rb, finalTransfer, finalInv } = await raceReceives(
      data.pendingTransfer.id, data.prod.id, data.w3.id,
    )
 ...
 'app/api/v1/warehouses/[id]/route.ts',
  ...
  it('webhooks are dispatched from sale and transfer-complete workflows', async () => {
    ...
    const transferReceiveRoute = await readFile('app/api/v1/transfers/[id]/receive/route.ts', 'utf-8')
